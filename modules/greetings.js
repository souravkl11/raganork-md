...
Asena.addCommand({pattern: 'welcome (.*)', fromMe: true, dontAddCommandList: true}, (async (message, match) => {
    if (match[1] === '') {
        ...
        if (match[1] === 'delete') { await message.client.sendMessage(message.jid,Lang.WELCOME_DELETED,MessageType.text); return await raganork.deleteMessage(message.jid, 'welcome'); }
        await raganork.setMessage(message.jid, 'welcome', match[1].replace(/#/g, '\n'));
...
Asena.addCommand({pattern: 'goodbye$', fromMe: true, desc: Lang.GOODBYE_DESC}, (async (message, match) => {
    var hg = await raganork.getMessage(message.jid, 'goodbye');
    if (hg === false) {
...
Asena.addCommand({pattern: 'goodbye (.*)', fromMe: true, dontAddCommandList: true}, (async (message, match) => {
    if (match[1] === '') {
        ...
        if (match[1] === 'delete') { await message.client.sendMessage(message.jid,Lang.GOODBYE_DELETED,MessageType.text); return await raganork.deleteMessage(message.jid, 'goodbye'); }
        await raganork.setMessage(message.jid, 'goodbye', match[1].replace(/#/g, '\n'));
