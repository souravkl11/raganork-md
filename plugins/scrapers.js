...
if (config.WORKTYPE == 'private') {
    ...
    skl.addCommand({pattern: 'song ?(.*)', fromMe: true, desc: Lang.SONG_DESC}, (async (message, match) => { 
		...
		const {data} = await axios(`https://zenzapi.xyz/api/play/playmp3?query=(match[1])&apikey=souravkl11`)
        const { result } = data
        if (match[1] === '') return await message.sendMessage('_Type a song name_');
        var souravkl11 = await axios.get(`${result.url}`, { responseType: 'arraybuffer' })
        await message.sendMessage(Buffer.from(souravkl11.data), MessageType.audio, { mimetype: Mimetype.mp3})
    ...
    skl.addCommand({ pattern: 'github ?(.*)', fromMe: true, desc: Glang.GİTHUB_DESC }, async (message, match) => {
    ...
    skl.addCommand({ pattern: 'github ?(.*)', fromMe: false, desc: Glang.GİTHUB_DESC }, async (message, match) => {
