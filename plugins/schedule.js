...
const { scheduledMessages } = require('./utils/db/schedulers'); // Note: This now imports the object with new methods
...
function parseTime(timeStr) {
    ...
    if (durationMatch) {
        ...
        return now.add(duration).toDate();
    }
...
Module({
    ...
    try {
        ...
        const timeFromNow = moment(scheduleTime).fromNow();
        const formattedTime = moment(scheduleTime).format('DD/MM/YYYY HH:mm');
...
Module({
    ...
    try {
        ...
        // Sort by scheduleTime to show upcoming messages first
        pending.sort((a, b) => a.scheduleTime.getTime() - b.scheduleTime.getTime());

        pending.forEach((msg, index) => {
            const timeFromNow = moment(msg.scheduleTime).fromNow();
            const formattedTime = moment(msg.scheduleTime).format('DD/MM/YYYY HH:mm');
