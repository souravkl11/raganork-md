...
if (Config.WORKTYPE == 'private') {
    ...
    Raganork.addCommand({ pattern: 'inchim ?(.*)', fromMe: true,dontAddCommandList: true }, (async (message, match) => {
    ...
    Raganork.addCommand({ pattern: 'png ?(.*)', fromMe: false,dontAddCommandList: true }, (async (message, match) => {

        if (match[1] === '') return await message.sendMessage(need);

        var ttinullimage = await axios.get(`https://api.zeks.me/api/emoji-image?apikey=TqFyJhzDvqftPnM63RQsWGwOdXZ&emoji=${encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
