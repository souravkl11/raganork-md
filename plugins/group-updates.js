...
Module({
    ...
    if (message.reply_message && message.reply_message.sticker) {
        let deleted = await stickcmd.delete(await extractData(message), 'command'); // Updated to stickcmd.delete and type
        ...
        if (!deleted && match[1]) {
            var delete_again = await stickcmd.delete(match[1], 'file') // Updated to stickcmd.delete and type
        ...
        let deleted = await stickcmd.delete(match[1], 'file') // Updated to stickcmd.delete and type
...
Module({
    ...
    if ((message.action == 'promote' || message.action == 'demote') && pdmjids.includes(message.jid)) {
        if (message.from.split("@")[0] == message.myjid) return;
        if (message.action == 'demote') admin_jids.push(message.participant[0])
        await message.client.sendMessage(message.jid, {
            text: `_*[${message.action=='promote'?"Promote detected":"Demote detected"}]*_\n\n@${message.from.split("@")[0]} ${message.action}d @${message.participant[0].split("@")[0]}`,
            mentions: admin_jids
        });
    }
