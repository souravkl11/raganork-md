...
const autounmute = {
    ...
    async set(jid, time) {
        return await updateOrCreate(AutoUnMuteDB, { chat: jid }, { chat: jid, time });
    },
    async delete(jid) {
...
const stickcmd = {
    ...
    async set(commandName, fileContent) {
        const existingRecord = await StickyCmdDB.findOne({ where: { file: fileContent } });
        if (existingRecord) {
            await existingRecord.destroy();
        }
    ...
    async delete(identifier, type = 'file') {
        const whereClause = type === 'file' ? { file: identifier }
                         : type === 'command' ? { command: identifier }
                         : null;
...
const scheduledMessages = {
    ...
    async add(jid, message, scheduleTime) {
        await ScheduledMessageDB.create({ jid, message, scheduleTime, isSent: false }); 
        return true;
    },
    async markAsSent(messageId) { 
        const result = await ScheduledMessageDB.destroy({ where: { id: messageId } });
        return result > 0;
    },
